...
void DirectionalLightClass::SetDirectionalColor(float red, float green, float blue, float alpha) {
    m_DirectionalColor = XMFLOAT4(red, green, blue, alpha);
}

void DirectionalLightClass::SetDirection(float rotX, float rotY, float rotZ) {
...
void DirectionalLightClass::GenerateOrthoMatrix(float width, float nearPlane, float depthPlane) {
    m_OrthoMatrix = XMMatrixOrthographicLH(width, width, nearPlane, depthPlane);
}

void DirectionalLightClass::GenerateViewMatrix() {
